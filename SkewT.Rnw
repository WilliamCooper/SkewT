...
thermodynamic diagrams (tephigram, emagram, pastagram, aerogram, St?ve
...
even in the Davies-Jones formula. For that reason, the goal here is
to construct new diagrams based on those two advances. For the pseudo-adiabatics,
...
where quantities with subscript $d$ refer to dry air (cf.\href{https://drive.google.com/file/d/0B1kIUH45ca5Ab2Z6cld1M1cydjA/view?usp=sharing}{ ProcessingAlgorithms.pdf},
p. 38).
...
XYplot <- function (.T, .p) {    
  return (data.frame(X=(.T-tBot) / (tTop-tBot) - log10(.p/pBot) / log10(pBot/pTop),                                        Y=log10(.p))) 
}
@
...
DLPLT2$Y2 <- DLPLT2$Y - (1-DLPLT2$X) * YR
g <- g + geom_text (data=DLPLT2, aes(x=X, y=Y, label=LABEL, angle=45, hjust=1.5, vjust=0.5),
                    size=4)
g <- g + geom_text (data=DLPLT2, aes(x=1.02, y=Y2, label=LABEL, angle=45, hjust=-0.3, vjust=-0.0),
                    size=3.5)
DLPLT3 <- DLPLT[(DLPLT$X <= 0), ]
DLPLT3 <- DLPLT3[(DLPLT3$X > -1), ]
DLPLT3$Y2 <- rep (log10(pTop), length (DLPLT3$X))
g <- g + geom_text (data=DLPLT3, aes(x=X+1, y=Y2, label=LABEL, angle=45, hjust=-0.2, vjust=0.2),
                    size=3.5)
...
g <- g + geom_path (data=MRDFI, aes(x=X, y=Y), color='red', lty=1, 
                    lwd=0.4, na.rm=TRUE)

## save the diagram for other uses
skewTDiagram <- g
# need pBot etc as reference for plotting an aircraft sounding relative to this background
save(pBot, pTop, tBot, tTop, skewTDiagram, file="./skewTDiagramC130.Rdata")
...
\attachm{\Sexpr{thisFileName}.Rnw\\\Sexpr{thisFileName}.pdf\\skewTDiagramC130.Rdata\\SessionInfo}
...
system (sprintf("zip %s.zip %s.Rnw %s.pdf skewTDiagramC130.Rdata SessionInfo", thisFileName, thisFileName, thisFileName))
