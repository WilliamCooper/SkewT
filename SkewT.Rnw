...
thermodynamic diagrams (tephigram, emagram, Stüve diagram) are standard
...
TQfromWBPT <- function (.T, .ThetaQ, .P) {
  return (.ThetaQ - WetEquivalentPotentialTemperature (.P, .T, MurphyKoop(.T)))
}
...
# add a wet-adiabat
ThetaQ <- WetEquivalentPotentialTemperature (1000, TWB[15], MurphyKoop(TWB[15]))
for (i in 1:length(pLevels)) {
  tt[i] <- nleqslv::nleqslv (10., TQfromWBPT, jac=NULL, ThetaQ, pLevels[i])$x
}
